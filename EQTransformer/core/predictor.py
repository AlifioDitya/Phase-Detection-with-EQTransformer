...
def _gen_predictor(new_list, args, model): 
    ...
    if args['estimate_uncertainty']:
        ...
        for batch in prediction_generator:
            inputs = batch['input']
            predD, predP, predS = model.predict(inputs, use_multiprocessing = args['use_multiprocessing'], workers = args['number_of_cpus'])
        ...
        for batch in prediction_generator:
            inputs = batch['input']
            pred_DD_mean, pred_PP_mean, pred_SS_mean = model.predict(inputs, use_multiprocessing = args['use_multiprocessing'], workers = args['number_of_cpus'])
